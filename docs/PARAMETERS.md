...
| **Context Scale** | 4x (Recommended) | None, 2x, 4x, 8x, 16x | Provides additional surrounding context at a lower resolution alongside the main tile. Helps the model understand broader tissue architecture. Hidden for MuViT (which handles multi-scale internally via level_scales). For how Resolution + Context Scale + context padding shape the actual tile geometry across training, the per-tile preview, and inference, see [Context and Downsample Design](CONTEXT_AND_DOWNSAMPLE_DESIGN.md). |
